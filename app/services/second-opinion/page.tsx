            ...
            {reasons.map(({ icon: Icon, title, description }, index) => (
              <RevealOnScroll as="li" key={title} delay={index * 0.06} className="list-none">
                <Card className="flex items-start gap-4 p-6">
                  <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-brand-teal-50 text-brand-teal-700">
                    <Icon className="h-5 w-5" />
                  </span>
                  <div>
                    <h2 className="font-bold text-brand-teal-900">{title}</h2>
                    <p className="mt-1 text-sm leading-relaxed text-brand-grey-500">{description}</p>
                  </div>
                </Card>
              </RevealOnScroll>
            ))}
