...
export default async function GalleryAlbumPage({ params }: PageProps) {
  ...
  return (
    <>
      ...
      <section className="bg-brand-teal-900 py-16">
        <Container>
          <RevealOnScroll>
      ...
      <section className="bg-white py-16">
        <Container className="flex flex-col gap-14">
          <div>
            <h2 className="mb-6 inline-flex items-center gap-2 text-lg font-bold text-brand-teal-900">
            ...
            {album.images.length > 0 ? (
              <StaggerGroup className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
                {album.images.map((src, index) => (
                  <RevealOnScroll as="li" key={src} delay={Math.min(index, 8) * 0.03} className="list-none">
                    <div className="relative aspect-square overflow-hidden rounded-2xl bg-brand-grey-100">
                      <Image
              ...
              <p className="text-sm text-brand-grey-400">Photos for this album are being uploaded — check back soon.</p>
            )}
          </div>
