...
export function TrustBar({
  ...
  return (
    <section className="border-y border-brand-grey-200 bg-white py-10">
      <Container className="mb-6 flex flex-col items-center gap-1 text-center">
        <p className="text-xs font-semibold uppercase tracking-[0.2em] text-brand-grey-400">
      ...
      <div className="relative overflow-hidden">
        <div className="flex w-max animate-marquee gap-10">
          {doubled.map(({ id, name, icon: Icon }, index) => (
            <span
              key={`${id}-${index}`}
              className="flex items-center gap-2 whitespace-nowrap rounded-full bg-brand-grey-50 px-5 py-2.5 text-sm font-semibold text-brand-teal-800"
            >
